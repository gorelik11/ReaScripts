-- RCBit LUFS Envelope Limiter V2.0
...
--
-- Dialog: FX scope, LUFS target, ceiling, attack, release, window, LUFS source (SWS/DryRun).
-- Dry run uses RENDER_STATS on time selection for post-FX LUFS measurement.
...
local scope_ret = reaper.ShowMessageBox(
  ...
  "RCBit LUFS Envelope Limiter V2.0 — FX Scope", 3)
...
local ok, csv = reaper.GetUserInputs(
  "RCBit LUFS Envelope Limiter V2.0 (" .. FX_SCOPE .. ")", 5,
...
-- Dry run LUFS measurement via RENDER_STATS
-- Sets time selection, selects track, runs dry run, parses LUFSI from result
local function measure_lufs_dryrun(track, start_time, end_time, mono)
  ...
  -- Set time selection and track selection
  reaper.GetSet_LoopTimeRange(true, false, start_time, end_time, false)
  reaper.SetOnlyTrackSelected(track)

  -- Configure for stem dry run
  reaper.GetSetProjectInfo(0, "RENDER_SETTINGS", 3, true)  -- stems of selected tracks

  -- Run dry run: 42448 = mono, 42439 = stereo
  ...
  -- Parse result: FILE:name;PEAK:val;LUFSI:val;...
  ...
  if not lufs_i then return nil, peak_db, "No LUFSI in dry run result: " .. result end
...
if not item then
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
  reaper.ShowMessageBox("No selected item.", "RCBit LUFS Envelope Limiter V2.0", 0)
  return
end
...
if not take or reaper.TakeIsMIDI(take) then
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
  reaper.ShowMessageBox("Selected item is not audio.", "RCBit LUFS Envelope Limiter V2.0", 0)
  return
end
...
if has_time_sel then
  ...
  if scan_end - scan_start < 0.001 then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
    reaper.ShowMessageBox("Time selection doesn't overlap item.", "RCBit LUFS Envelope Limiter V2.0", 0)
    return
  end
end
...
local lufs_source_label = LUFS_SOURCE
...
if LUFS_SOURCE == "SWS" then
  ...
  if not lufs_ok2 or not lufs_integrated or lufs_integrated <= -200 then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
    reaper.ShowMessageBox("LUFS analysis failed (SWS).", "RCBit LUFS Envelope Limiter V2.0", 0)
    return
  end
  ...
  -- Dry run: must temporarily allow UI refresh
  reaper.PreventUIRefresh(-1)
  ...
  if not dr_lufs then
    ...
    reaper.ShowMessageBox("Dry run LUFS failed:\n" .. (dr_err or "unknown"),
      "RCBit LUFS Envelope Limiter V2.0", 0)
    return
  end
...
if peak_win_count == 0 then
  -- No peaks exceed ceiling after LUFS gain — apply static LUFS gain
  local lufs_m, lufs_br = calc_rcbit_params(lufs_gain_db, false)
  if not lufs_m then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V2 - no changes", -1)
  ...
  reaper.Undo_EndBlock("RCBit LUFS Envelope Limiter V2.0", -1)
  reaper.ShowMessageBox(
...
-- Assign gains: peak regions get ceiling-relative reduction, clean regions get LUFS gain
for _, reg in ipairs(regions) do
...
if fx_idx < 0 then
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
  reaper.ShowMessageBox("Could not add RCBitRangeGain.", "RCBit LUFS Envelope Limiter V2.0", 0)
  return
end
...
if not get_env then
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
  reaper.ShowMessageBox(env_err or "Cannot create envelope", "RCBit LUFS Envelope Limiter V2.0", 0)
  return
end
...
if not env_macro or not env_br then
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit LUFS Env V2 - failed", -1)
  reaper.ShowMessageBox("Failed to create envelopes.", "RCBit LUFS Envelope Limiter V2.0", 0)
  return
end
...
local summary = string.format(
  "RCBit LUFS Envelope Limiter V2.0 — Done\n\n"
  .. "FX Scope: %s | LUFS Source: %s\n"
  .. "LUFS: %.1f → %d (gain: %+.2f dB, eff: %+.2f dB)\n"
...
reaper.UpdateArrange()
reaper.Undo_EndBlock("RCBit LUFS Envelope Limiter V2.0", -1)
reaper.ShowMessageBox(summary, "RCBit LUFS Envelope Limiter V2.0", 0)
