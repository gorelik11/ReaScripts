-- RCBit Limiter V7.0 Headless
-- Peak-only limiter (no LUFS) with floor BR quantization
-- Reads params from ~/rcbit_limiter_v7_params.txt, writes to ~/rcbit_limiter_v7_results.txt
...
local params_file = os.getenv("HOME") .. "/rcbit_limiter_v7_params.txt"
local results_file = os.getenv("HOME") .. "/rcbit_limiter_v7_results.txt"
...
local function calc_rcbit_params(gain_db)
  ...
  bit_ratio = math.floor(bit_ratio / BR_STEP) * BR_STEP  -- floor for peak safety
...
if not item then
  ...
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit Limiter V7 Headless - failed", -1)
  return
end
...
if peak_win_count == 0 then
  ...
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit Limiter V7 Headless - no changes", -1)
  return
end
