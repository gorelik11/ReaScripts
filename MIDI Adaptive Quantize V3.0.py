#!/usr/bin/env python3
"""MIDI Adaptive Quantize V3.0 -- late-only, polyphony-safe MIDI note quantize.

Same dialog/GUI as V2.0; the only change is the planning core: movement is now
decided per exact-onset cluster (see plan_note_moves / _onset_clusters), so a
single late voice in a chord no longer drags its already-clean chord-mates, and
early notes are never pushed later.
"""
...
def plan_note_moves(onsets, families, qn_of_time, time_of_qn,
    ...
    Nearby onsets still share grid-family detection (straight vs triplet is
    chosen over the whole gap-group), but movement is planned per exact onset
    cluster. This keeps a late note from dragging neighboring notes that were
    already clean, and never moves early notes later.

    Returns a list of {"onsets": [cluster times], "move": seconds}; only clusters
    that exceed the threshold (and pass the max-move guard) appear.
    """
    ...
    for g in groups:
        ...
        for cluster in _onset_clusters(g):
            t = cluster[0]
            tq = qn_of_time(t)
            ...
            planned.append({"onsets": list(cluster), "move": move})
    return planned
...
def _open_dialog():
    ...
    try:
        st = _load_defaults()
        ctx = _ImGui.CreateContext("MIDI Adaptive Quantize V3")
        _GA = {
            "imgui": _ImGui,
            "ctx": ctx,
            "ui": {
...
def _ga_frame():
    ...
    visible, open_ = ImGui.Begin(ctx, "MIDI Adaptive Quantize V3", True)
    apply_clicked = False
