...
- **Numerically pre-validated (not guessed):** the 5.551e-17 identity, raw branch peaks 0.6417/0.7968, and stereo equivalences are printed by `shelf_modeb_proto.py`; the 0.249999/0.250000 clamped-branch peaks come from the branch-recovery formula (`branch[n-L] + (y[n]-x[n-L])`) run against that same prototype function on 2026-07-04 — the identical formula the Task 1/2 tests use.
- **Adversarial plan review (2026-07-04): EXECUTE AS-IS.** All risks (transcription fidelity, two-gate completeness, reconstruction/false-green, oracle arithmetic/RED-ness, EEL2/ASCII) verified against real code + a prototype run. Noted: the two identity tests pass for any tap (bda cancels), so tap correctness rests on the two branch-clamp tests — adequate, but the tap is the least-redundantly-covered surface if a live issue appears.
