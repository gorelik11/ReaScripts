-- RCBit LUFS Envelope Limiter V3.0 Headless
...
--
-- Reads params from ~/rcbit_lufs_env_v3_params.txt
-- Writes results to ~/rcbit_lufs_env_v3_results.txt
--
-- Params:
--   TARGET_LUFS   (target LUFS, e.g. -9)
...
--   LUFS_SOURCE   (SWS, MonoDryRun, or StereoDryRun, default SWS)
...
local params_file = os.getenv("HOME") .. "/rcbit_lufs_env_v3_params.txt"
local results_file = os.getenv("HOME") .. "/rcbit_lufs_env_v3_results.txt"
...
local TARGET_LUFS = tonumber(params.TARGET_LUFS) or -9
...
local LUFS_SOURCE = params.LUFS_SOURCE or "SWS"
...
local function measure_lufs_dryrun(track2, start_time, end_time, mono)
...
if LUFS_SOURCE == "SWS" then
  ...
  if not lufs_ok2 or not lufs_integrated or lufs_integrated <= -200 then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V3 - failed", -1)
    write_result("ERROR: LUFS analysis failed (SWS).")
    return
  end
  ...
  if not dr_lufs then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V3 - failed", -1)
    write_result("ERROR: Dry run LUFS failed: " .. (dr_err or "unknown"))
    return
  end
...
if peak_win_count == 0 then
  ...
  if not lufs_m then
    ...
    write_result(string.format("OK: Already near target LUFS (%.1f, gain=%.2fdB). Nothing to do.",
  ...
  write_result(string.format(
    "OK: No peaks. Static LUFS gain. LUFS:%.1f->%d gain:%+.2fdB eff:%+.2fdB Macro=%d BR=%.2f Scope=%s",
    current_lufs, TARGET_LUFS, lufs_gain_db, eff, lufs_m, lufs_br, FX_SCOPE))
  return
end
...
elseif LIMITER_MODE == "Micro" then
  ...
  if fx2 < 0 then
    reaper.PreventUIRefresh(-1)
    reaper.Undo_EndBlock("RCBit LUFS Env V3 - failed", -1)
    write_result("ERROR: Could not add limiter RCBitRangeGain.")
    return
  end
  fx_count = fx_count + 1
  ...
  if lufs_m then
    env_points_info = env_points_info .. string.format(" LUFSinst:M=%d,BR=%.2f", lufs_m, lufs_br)
  end
end
...
local result = string.format(
  "OK: Scope=%s Mode=%s LUFSSrc=%s LUFS:%.1f->%d gain:%+.2fdB eff:%+.2fdB "
  .. "Ceiling=%.1f Atk=%dms Rel=%dms Win=%dms "
  .. "CH=%d SR=%d FX=%d Windows=%d(%dpeak) Regions=%d(%dpeak,%dclean) %s",
...
if dryrun_peak then
  result = result .. string.format(" DryRunPeak:%.2fdB", dryrun_peak)
end
if has_time_sel then
