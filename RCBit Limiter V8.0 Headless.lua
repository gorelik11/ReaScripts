-- RCBit Limiter V8.0 Headless
-- Peak-only limiter with accessor-based post-split verification
-- Reads params from ~/rcbit_limiter_v8_params.txt, writes to ~/rcbit_limiter_v8_results.txt
--
...
--   - Eliminates BR quantization overshoot
...
local params_file = os.getenv("HOME") .. "/rcbit_limiter_v8_params.txt"
local results_file = os.getenv("HOME") .. "/rcbit_limiter_v8_results.txt"
...
if not item then
  ...
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit Limiter V8 Headless - failed", -1)
  return
end
...
if peak_win_count == 0 then
  write_result(string.format("OK\nNo peaks exceeding %.1f dB", CEILING_DB))
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("RCBit Limiter V8 Headless - no changes", -1)
  return
end
    ...
    if tk and reaper.TakeFX_GetCount(tk) > 0 then
      ...
      if src_peak_db > -200 then
        ...
        if post_fx_peak > CEILING_DB + VERIFY_TOLERANCE then
          local fixed = false
          -- Peak limiter always uses negative macro (reduction)
          if macro < 0 then
