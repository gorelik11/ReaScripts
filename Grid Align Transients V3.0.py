#!/usr/bin/env python3
"""Grid Align Transients V3.0 - adaptive transient quantizer with a ReaImGui dialog."""
...
def _apply_group_edit(item_id, item_pos, item_len, seg_start, seg_end, move,
                      bounds):
    """Split [seg_start, seg_end] out of the item and shift it by `move`.

    Returns {"moved": id|None, "left": id|None, "right": id|None, "clipped":
    bool}. The caller applies groups right-to-left so `item_id` stays valid for
    groups further left. Both splits are checked: V2 ignored the second return
    value, so a failed tail split silently left `mid` running to the end of the
    item and moved the whole remainder (audit P1-4).
    """
    lo, hi = bounds
    raw_start, raw_end = seg_start, seg_end
    seg_start = max(seg_start, item_pos + _EDGE_EPS, lo)
    seg_end = min(seg_end, item_pos + item_len - _EDGE_EPS, hi)
    clipped = (seg_start > raw_start + 1e-9) or (seg_end < raw_end - 1e-9)
    none_result = {"moved": None, "left": None, "right": None, "clipped": clipped}
    if seg_end - seg_start < _EDGE_EPS:
        return none_result
    mid = RPR_SplitMediaItem(item_id, seg_start)   # noqa: F821
    if not mid:
        return none_result
    tail = RPR_SplitMediaItem(mid, seg_end)        # noqa: F821
    if not tail:
        # Could not carve a bounded segment: refuse to move a piece that runs
        # to the end of the item, which would drag the whole remainder along.
        return {"moved": None, "left": item_id, "right": None, "clipped": clipped}
    ...
    return {"moved": mid, "left": item_id, "right": tail, "clipped": clipped}


def _source_room_after(item_id):
    """Seconds of source material available past an item's current end."""
    take = RPR_GetActiveTake(item_id)              # noqa: F821
    if not take:
        return 0.0
    src_len = _source_length(item_id)
    if src_len <= 0:
        return 0.0
    offs = RPR_GetMediaItemTakeInfo_Value(take, "D_STARTOFFS")   # noqa: F821
    length = RPR_GetMediaItemInfo_Value(item_id, "D_LENGTH")     # noqa: F821
    return max(0.0, src_len - offs - length)


def _close_gap_pair(left_id, moved_id, right_id, bounds, crossfade_ms=_CROSSFADE_MS):
    """Close the gap a moved segment left behind, touching ONLY its siblings.

    left_id / right_id are the pieces the split produced around moved_id; either
    may be None at an item edge. `bounds` is (lo, hi) - the original item window
    intersected with any active time selection. No edge is ever moved or
    extended past it, and no item outside these three ids is read or written.
    V2 swept the whole track here, which let it edit unrelated material and
    cross the time selection (audit P1-2, P2-2).
    """
    ...
    if moved_id is None:
        return 0
    m_pos = RPR_GetMediaItemInfo_Value(moved_id, "D_POSITION")      # noqa: F821
    m_len = RPR_GetMediaItemInfo_Value(moved_id, "D_LENGTH")        # noqa: F821
    ...
    if left_id is not None:
        ...
        gap = m_pos - (l_pos + l_len)
        if gap < -1e-4:
            # The segment moved LEFT onto its sibling: trim the sibling back to
            # the segment start (plus a crossfade) instead of leaving two items
            # overlapping. V2 only ever handled gap > 0 and left the overlap in
            # place, which is what doubled material on the timeline.
            new_len = max(0.0, (m_pos + crossfade) - l_pos)
            if new_len < l_len:
                RPR_SetMediaItemInfo_Value(left_id, "D_LENGTH", new_len)  # noqa: F821
                filled += 1
        elif gap > 1e-4:
            want = gap + crossfade
            room = max(0.0, min(hi, m_pos + crossfade) - (l_pos + l_len))
            grab = min(want, room, _source_room_after(left_id))
            if grab > 0:
                RPR_SetMediaItemInfo_Value(left_id, "D_LENGTH", l_len + grab)  # noqa: F821
                filled += 1
    ...
    if right_id is not None:
        ...
        gap = r_pos - (m_pos + m_len)
        if gap < -1e-4:
            # The segment moved RIGHT onto its sibling (early attack snapped
            # later): trim the sibling's LEFT edge forward so the two stop
            # overlapping. This is the mirror of the left-overlap case; without
            # it the moved segment and the sibling sit as parallel duplicates.
            take = RPR_GetActiveTake(right_id)                      # noqa: F821
            if take:
                offs = RPR_GetMediaItemTakeInfo_Value(take, "D_STARTOFFS")  # noqa: F821
                new_start = m_pos + m_len - crossfade
                trim = new_start - r_pos
                if 0 < trim < r_len:
                    RPR_SetMediaItemTakeInfo_Value(take, "D_STARTOFFS", offs + trim)  # noqa: F821
                    RPR_SetMediaItemInfo_Value(right_id, "D_POSITION", new_start)     # noqa: F821
                    RPR_SetMediaItemInfo_Value(right_id, "D_LENGTH", r_len - trim)     # noqa: F821
                    filled += 1
        elif gap > 1e-4:
            take = RPR_GetActiveTake(right_id)                      # noqa: F821
            if take:
                offs = RPR_GetMediaItemTakeInfo_Value(take, "D_STARTOFFS")  # noqa: F821
                want = gap + crossfade
                room = max(0.0, r_pos - max(lo, m_pos + m_len - crossfade))
                grab = min(want, room, offs)
                if grab > 0:
                    RPR_SetMediaItemTakeInfo_Value(take, "D_STARTOFFS", offs - grab)  # noqa: F821
                    RPR_SetMediaItemInfo_Value(right_id, "D_POSITION", r_pos - grab)  # noqa: F821
                    RPR_SetMediaItemInfo_Value(right_id, "D_LENGTH", r_len + grab)    # noqa: F821
                    filled += 1
    return filled
...
def _open_dialog():
    ...
    try:
        st = _load_defaults()
        ctx = _ImGui.CreateContext("Grid Align Transients V3")
        _GA = {
            "imgui": _ImGui,
            "ctx": ctx,
            "ui": {
...
def _ga_frame_body(g):
    ...
    visible, open_ = ImGui.Begin(ctx, "Grid Align Transients V3", True)
    apply_clicked = False
...
def _plan_item_segments(m, time_sel, families_for, grid_step_for, gap_for,
    ...
    Returns (segments, prev_lag, collided). Each segment is
    {seg_start, seg_end, move}; prev_lag is threaded through for adaptive
    chaining; collided is True if any move was refused because it would land on
    material that is still in place.
    """
    collided = False
    item_pos, item_len = m["position"], m["length"]
    win = compute_analysis_window(item_pos, item_len, m["start_offs"], time_sel)
    if win is None:
        return [], prev_lag, collided
    ...
    # Anchor = FIRST attack of each group. The ear tracks the leading transient
    # of a flam/roll; the rest follow it rigidly. V2 anchored on the largest
    # error, letting one noisy detection drag correct hits away (audit P1-3).
    anchors = [g[0] for g in groups]
    fams = [select_family_positions(
                families,
                choose_family_for_group([qn_of_time(t) for t in g], families))
            for g in groups]
    edits, prev_lag = plan_corrections(anchors, fams, qn_of_time, time_of_qn,
                                       threshold_s, mode, grid_step_for,
                                       prev_lag)
    moves = {e["time"]: e["move"] for e in edits}
    ...
    for g in groups:
        ...
        if move is None:
            continue
        seg_start = g[0] - _SPLIT_PREROLL
        seg_end = g[-1] + _SEG_TAIL
        # Overlapping the neighbouring material of the SAME item is normal: the
        # sibling gets trimmed during cleanup. What must be refused is a move
        # that escapes the item / time selection, or lands on ANOTHER planned
        # segment - that would destroy a correction we already decided on
        # (audit P1-1).
        occupied = [(p["seg_start"] + p["move"], p["seg_end"] + p["move"])
                    for p in planned]
        escapes = (seg_start + move < item_pos - 1e-9
                   or seg_end + move > item_pos + item_len + 1e-9)
        if escapes or segment_collides(seg_start, seg_end, move, occupied):
            collided = True
            continue
        planned.append({"seg_start": seg_start, "seg_end": seg_end, "move": move})
    ...
    return planned, prev_lag, collided
...
def _run_in_reaper(config, show_report=False):
    ...
    try:
        if source_mode == "splits":
            ...
            plans = []
            for _tr, group in _group_by_track(ok):
                for m in sorted(group, key=lambda m: m["position"]):
                    # Groove is a local property: never carry a lag across an
                    # item or a track boundary (audit P2-1).
                    prev_lag = None
                    segs, prev_lag, collided = _plan_item_segments(
            ...
            for m, segs in plans:
                lo = m["position"]
                hi = m["position"] + m["length"]
                if time_sel:
                    lo = max(lo, time_sel[0])
                    hi = min(hi, time_sel[1])
                for p in sorted(segs, key=lambda s: s["seg_start"], reverse=True):
                    res = _apply_group_edit(m["item_id"], m["position"],
                                            m["length"], p["seg_start"],
                                            p["seg_end"], p["move"], (lo, hi))
                    if res["clipped"]:
                        crossed_time_selection = True
                    if res["moved"]:
                        moved_ids.add(res["moved"])
                        edited += 1
                        # cleanup is scoped to the two siblings of THIS split
                        _close_gap_pair(res["left"], res["moved"],
                                        res["right"], (lo, hi))
    finally:
        RPR_PreventUIRefresh(-1)   # noqa: F821
        RPR_UpdateArrange()        # noqa: F821
        RPR_Undo_EndBlock("Grid Align Transients V3.0", -1)  # noqa: F821

    report = {"edited_segments": edited, "skipped": skipped,
              "neighbor_touched": neighbor_touched,
              "crossed_time_selection": crossed_time_selection}
    if show_report:
        RPR_ShowMessageBox(  # noqa: F821
            "Grid Align Transients V3.0\n\n"
