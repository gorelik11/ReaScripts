-- Batch Apply Limiter V2.0
-- Apply a limiter script to multiple selected items sequentially.
-- Dialog 1: choose limiter + edit params before processing.
...
--   Env  V2: ~/env_limiter_v2_params.txt
...
local LIMITERS = {
  { name = "LUFS V8",
    named_id = "RS72928ed6d26d21c6144d6783cf9e05c354d76e92",
    params_file = os.getenv("HOME") .. "/rcbit_v8_params.txt",
    fields = {"TARGET_LUFS", "CEILING_DB", "ATTACK_MS", "RELEASE_MS", "WINDOW_MS"},
    defaults = {"-9", "-0.5", "0", "70", "5"},
  },
  { name = "Lim V8",
    named_id = "RS5651d862b4148716f74908abb57c1a2cfd7a9960",
    params_file = os.getenv("HOME") .. "/rcbit_limiter_v8_params.txt",
    fields = {"CEILING_DB", "ATTACK_MS", "RELEASE_MS", "WINDOW_MS"},
    defaults = {"-0.5", "0", "70", "5"},
  },
  { name = "LUFS V6",
    named_id = "RS4ea15987de3ed846432f69e3d60adc9ad58169d9",
    params_file = os.getenv("HOME") .. "/rcbit_v6_params.txt",
    fields = {"TARGET_LUFS", "CEILING_DB", "ATTACK_MS", "RELEASE_MS", "WINDOW_MS"},
    defaults = {"-9", "-0.5", "0", "70", "5"},
  },
  { name = "Env V2",
    named_id = "RS8f4e25e7c9483f84b16046235f77e72409c75f4d",
    params_file = os.getenv("HOME") .. "/env_limiter_v2_params.txt",
    fields = {"CEILING_DB", "ATTACK_MS", "RELEASE_MS", "WINDOW_MS"},
    defaults = {"-0.5", "0", "70", "5"},
  },
}
...
local retval, input = reaper.GetUserInputs(
  ...
  table.concat(names, "  ") .. ":,extrawidth=60",
...
if f then
  for line in f:lines() do
    ...
    if key then current[key] = val end
  end
  f:close()
end
...
for v in input2:gmatch("([^,]+)") do
  new_values[#new_values + 1] = v:match("^%s*(.-)%s*$")  -- trim
end
...
-- Store items: position + track GUID (robust across splits)
